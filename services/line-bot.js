    ...
    async function handlePostback(event, client) {
        ...
        if (userId) {
            try {
                ...
                // 更新或建立 staff 記錄
                db.prepare(`
                    INSERT INTO staff (user_id, display_name, updated_at)
                    VALUES (?, ?, datetime('now'))
        ...
        if (action === 'sold' && inventoryId) {
            ...
            // 📋 記錄操作日誌
            db.prepare(`
                INSERT INTO operation_logs (user_id, user_name, action, inventory_id, product_name, details, source)
                VALUES (?, ?, 'sold', ?, ?, '到期前售出', 'line')
            `).run(userId || 'unknown', userName, inventoryId, productName);
        ...
        if (action === 'remove' && inventoryId) {
            ...
            db.prepare(`
                UPDATE inventory 
                SET status = 'disposed', updated_at = datetime('now')
                WHERE id = ?
            `).run(inventoryId);
                ...
                messages: [{
                    ...
                    contents: {
                        ...
                        footer: {
                            ...
                            contents: [{
                                type: 'button',
                                action: { type: 'uri', label: '👉 點我私訊拍照', uri: `https://line.me/R/oaMessage/${LINE_OA_ID}/?拍照` },
                                style: 'primary',
                                color: '#1DB446',
                                height: 'sm'
                            }]
                    ...
                    messages: [{
                        ...
                        contents: {
                            ...
                            footer: {
                                ...
                                contents: [{
                                    type: 'button',
                                    action: { type: 'uri', label: '👉 點我私訊抽籤', uri: `https://line.me/R/oaMessage/${LINE_OA_ID}/?抽` },
                                    style: 'primary',
                                    color: '#FF6B35',
                                    height: 'sm'
                                }]
                ...
                contents: [
                    {
                        ...
                        action: { 
                            type: 'postback', 
                            label: '🔍 找不到（已售出）', 
                            data: `action=sold&id=${item.id}`, 
                            displayText: `「${item.name}」找不到了，應該已售出 ✨` 
                        },
                        style: 'secondary', 
                        height: 'sm'
                    },
