...
CREATE TABLE IF NOT EXISTS operation_logs (
    ...
    user_id TEXT NOT NULL,                 -- LINE userId
    user_name TEXT,                        -- 顯示名稱
    action TEXT NOT NULL,                  -- 動作：register/sold/disposed/photo
    inventory_id INTEGER,                  -- 關聯庫存 ID
    product_name TEXT,                     -- 商品名稱
    details TEXT,                          -- 額外資訊
    source TEXT DEFAULT 'line',            -- 來源：line/web
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (inventory_id) REFERENCES inventory(id)
);

-- 📊 店員資料表（LINE userId 對應名字）
CREATE TABLE IF NOT EXISTS staff (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    user_id TEXT UNIQUE NOT NULL,          -- LINE userId
    display_name TEXT,                     -- LINE 顯示名稱
    nickname TEXT,                         -- 自訂暱稱（店長可改）
    role TEXT DEFAULT 'staff',             -- 角色：admin/staff
    is_active INTEGER DEFAULT 1,           -- 是否在職
...
CREATE INDEX IF NOT EXISTS idx_operation_logs_date ON operation_logs(created_at);
